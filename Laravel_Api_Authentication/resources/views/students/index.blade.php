        ...
        <div class="card-header d-flex">
            ...
            <div class="col-5 offset-5">
                <form action="{{ route('student.index') }}" method="GET">
                    @csrf
                    <div class="row">
                        <input type="text" class="form-control col" value="{{ request('searchName') }}"
                            name="searchName" placeholder="Search">
                        <button type="submit" class="btn btn-outline-info col-2 mx-2">Search</button>
                    </div>
                </form>
            </div>
        </div>
            ...
            <table class="table table-strip">
                ...
                <tbody>
                    @if (is_countable($students) && count($students) > 0)
                        ...
                        @endforeach
                    @else
                        <small class="text-danger">There is no Data...</small>
                    @endif
                </tbody>
            </table>
        </div>
